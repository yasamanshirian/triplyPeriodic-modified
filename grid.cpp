  ...
  for (int k=in_klo;k<=in_khi;k++)
    for (int j=in_jlo;j<=out_jhi;j++)
      for (int i=in_ilo;i<=out_ihi;i++)
         {
          U_fft.x[count].im=0;
          U_fft.x[count].re=V_LES.x(i-in_ilo+bs_,j-in_jlo+bs_,k-in_klo+bs_);
	  
          U_fft.y[count].im=0;
          U_fft.y[count].re=V_LES.y(i-in_ilo+bs_,j-in_jlo+bs_,k-in_klo+bs_);
	  
          U_fft.z[count].im=0;
          U_fft.z[count++].re=V_LES.z(i-in_ilo+bs_,j-in_jlo+bs_,k-in_klo+bs_);
...
void grid::Update_RV_LES_WOQ()
{
  ...
  {     //std::cout<<"inside sgs"<<std::endl;
  	SubGridOperator();
  	RHS_RV.Equal_LinComb(param_->eta0()/3.,dummy2,param_->eta0(), dummy,param_->eta_sgs(),OperatorSGS);
  }
  else RHS_RV.Equal_LinComb(param_->eta0()/3.,dummy2,param_->eta0(),dummy); //RHS = -mp/Vcell*RHS + mu/3*grad(div(U)) + mu*div(grad(U))
